...
* Change cookies from pastebin or anywhere that support raw/plain text view

## 🤔 How to use
Go to part of this code:
```javascript
const config = {
	cookie: "xxx",
	reactType: [1,16,2,3],
...
```
### 🚶‍♂️‍➡️ Steps:
- Change the `"xxx"` to your copied Facebook cookies, like: `cookie: "sb=xxxxxx;datr=xxxxxxxxx;c_user=xxxxxxxx",` (don't remove the double-tick) or you can use website that can view raw/plain text like [Pastebin](https://pastebin.com) and it's goes like: `cookie: "https://pastebin.com/raw/xxxxxxxx",`
- Change `reactType` to reacts (with React ID) you want, depends on the table below. Example: `reactType: [1,16,2,3],` (without double-tick/single-tick, separated with commas. It will randomize executing LIKE, LOVE, WOW and CARE react every single post. If you want single react, it's goes like: `reactType: [1],`. That mean only LIKE react will be executed).
